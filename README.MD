...
## Installation
While in the directory to the your the root of your bot folder (the file path you copied). IE: `cd C:\repos\passel-js` where `C:\repos\passel-js` is the folder you extracted the contents of the repo .zip file that was downloaded. You are at the root of the folder if you see the `package.json` and `package-lock.json` files.
